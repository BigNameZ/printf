...
{ /* char is stored at left and paddind at buffer's right */
	...
	if (width > 1)
	{
		...
		for (v = 0; v < width - 1; v++)
			buffer[BUFF_SIZE - v - 2] = padd;

		if (flags & F_MINUS)
			return (write(1, &buffer[0], 1) +
					write(1, &buffer[BUFF_SIZE - v - 1], width - 1));
		else
			return (write(1, &buffer[BUFF_SIZE - v - 1], width - 1) +
					write(1, &buffer[0], 1));
	}
	...
	int width, int flags, char padd, char extra_c, int padd_start)
{
	...
	if (width > length)
	{
		...
		if (flags & F_MINUS && padd == ' ')/* Asign extra char to left of buffer */
		{
			...
			if (extra_c)
				buffer[--ind] = extra_c;
			return (write(1, &buffer[ind], length) + write(1, &buffer[3], v - 3));
		}
		else if (!(flags & F_MINUS) && padd == ' ')/* extra char to left of buffer */
		{
			...
			if (extra_c)
				buffer[--ind] = extra_c;
			return (write(1, &buffer[3], v - 3) + write(1, &buffer[ind], length));
		}
		else if (!(flags & F_MINUS) && padd == '0')/* extra char to left of padd */
		{
			if (extra_c)
